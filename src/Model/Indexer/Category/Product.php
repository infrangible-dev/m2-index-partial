    ...
    extends Indexer
{
    ...
    protected function performReindex()
    {
        ...
        $this->logging->debug(sprintf('Updating category product index for %d article(s)', count($allEntityIds)));

        if ( ! $this->isTest()) {
            $this->getIndexer()->reindexList($allEntityIds);
        }

        $this->logging->info(sprintf('Category product index was updated for %d article(s)', count($allEntityIds)));
    }
