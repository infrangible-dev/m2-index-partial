    ...
    extends Indexer
{
    ...
    protected function performReindex()
    {
        ...
        $this->logging->debug(sprintf('Updating price index for %d article(s)', count($allEntityIds)));

        if ( ! $this->isTest()) {
            $this->getIndexer()->reindexList($allEntityIds);
        }

        $this->logging->info(sprintf('Price index was updated for %d article(s)', count($allEntityIds)));
    }
