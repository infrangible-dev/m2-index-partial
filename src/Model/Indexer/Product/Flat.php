    ...
    extends Indexer
{
    ...
    protected function performReindex()
    {
        ...
        $this->logging->debug(sprintf('Updating product flat index for %d article(s)', count($allEntityIds)));

        if ( ! $this->isTest()) {
            $this->getIndexer()->reindexList($allEntityIds);
        }

        $this->logging->info(sprintf('Product flat index was removed of %d article(s)', count($allEntityIds)));
    }
