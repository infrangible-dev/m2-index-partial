    ...
    extends Indexer
{
    ...
    protected function performReindex()
    {
        ...
        $this->logging->debug(sprintf('Updating product category index for %d article(s)', count($allEntityIds)));

        if ( ! $this->isTest()) {
            $this->getIndexer()->reindexList($allEntityIds);
        }

        $this->logging->info(sprintf('Product category index was updated for %d article(s)', count($allEntityIds)));
    }
